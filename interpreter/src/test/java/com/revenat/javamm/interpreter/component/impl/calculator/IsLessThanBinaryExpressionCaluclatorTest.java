...
class IsLessThanBinaryExpressionCaluclatorTest extends AbstractBinaryExpressionmCalculatorTest {

    @Test
    @Order(1)
    void shouldSupportLessThanBianryOperator() {
        assertThat(calculator.getOperator(), is(BinaryOperator.PREDICATE_LESS_THAN));
    }
    ...
    @ParameterizedTest
    @MethodSource("unsuportedPairsProvider")
    @Order(3)
    ...
    static Stream<Arguments> unsuportedPairsProvider() {
        return Stream.of(
    ...
    protected BinaryExpressionCalculator createCalculatorUnderTest() {
        return new IsLessThanBinaryExpressionCaluclator();
    }
}
