...
class IsLessThanOrEqualsBinaryExpressionCalculatorTest extends AbstractBinaryExpressionCalculatorTest {
    ...
    @Test
    @Order(1)
    void shouldSupportLessThanOrEqualsOperator() {
        assertCalculatorSupportsOperator(calculator, BinaryOperator.PREDICATE_LESS_THAN_OR_EQUALS);
    }

    @Test
    @Order(2)
    void shouldCalculateLessThanOrEqualsOperationForNumbers() {
        assertThat(calculate(0, 5), is(true));
        assertThat(calculate(0, 0.0001), is(true));
        assertThat(calculate(0.0, 0), is(true));
        assertThat(calculate(10.999998, 10.999999), is(true));
        assertThat(calculate(15, 5), is(false));
        assertThat(calculate(5.5, 5), is(false));
    }

    @ParameterizedTest
    @MethodSource("unsupportedPairsProvider")
    @Order(3)
    void shouldFailToCalculateLessThanOrEqualsOperationForTypesOtherThanNumbers(final Object value1, final Object value2) {
    ...
    protected BinaryExpressionCalculator createCalculatorUnderTest() {
        return new IsLessThanOrEqualsBinaryExpressionCalculator();
    }
}
